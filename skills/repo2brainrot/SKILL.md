...
2. **Generate all videos** by running the pipeline script:
   ```
   python skills/repo2brainrot/scripts/generate_video.py --scripts output/<repo-name>/scripts/ --assets assets/ --output output/<repo-name>/videos/
   ```

If this skill is installed outside a clone of this repository, use the bundled `scripts/` directory from the installed skill location instead of the example repo-relative path above.
